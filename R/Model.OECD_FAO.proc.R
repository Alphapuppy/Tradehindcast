...
#This file will output the share of crops used for biofuels
cropbioshare <- function(){

oecddatadir <- paste0(datadir, "OECD_FAO/")
...
basedata.cropbioshare <- starch %>% dplyr::select(reg, crop, year, crop.consumption, bioshare) %>% 
  ...
  mutate_at(vars(crop.consumption, biocrop), MA.5) %>% 
  ...
  filter(year %in% study.year) %>% filter(variable == "bioshare")
